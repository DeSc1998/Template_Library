...
namespace ds {
	...
	public:
		...
		integer( longInt value, const std::wstring& s ) : symbols(s) {
			if (value < 0) {
				value = -value;
				is_negativ = !is_negativ;
			}
		...
		integer& operator = ( longInt value ) {
			if (value < 0) {
				value = -value;
				is_negativ = !is_negativ;
			}
		...
		integer operator + ( const integer& n ) const {
			if (n.is_negativ && this->is_negativ)
				goto calc_add;
			else if (n.is_negativ) {
		...
		integer operator * ( longInt n ) const {
			integer tmp = *this;
